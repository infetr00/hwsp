...
class UsuarioController {
	...
	def delete = {
		...
		if (person) {
			...
			if (!(authPrincipal instanceof String) && authPrincipal.username == person.username) {
				...
				Rol.findAll().each { it.removeFromPeople(person) }
				person.delete()
	...
	def create = {
		[person: new Usuario(params), authorityList: Rol.list()]
	}
	...
	def save = {
		...
		if (person.save()) {
			...
			render view: 'create', model: [authorityList: Rol.list(), person: person]
		}
	}
	...
	private void addRoles(person) {
		for (String key in params.keySet()) {
			if (key.contains('ROLE') && 'on' == params.get(key)) {
				Rol.findByAuthority(key).addToPeople(person)
			}
		}
	}
